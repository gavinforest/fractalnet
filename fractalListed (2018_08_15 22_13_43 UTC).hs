...
vdot :: [Float] -> [Float] -> Float
vdot x y = foldl' (+) 0.0 $ zipWith (*) x y
...
layer input consol branching w b =  map (tanh) $ zipWith (+) b $ (\x -> floatConsol x consol) $ zipWith (*) w branched
    where branched = concatMap (replicate branching) input
       
feedForward :: V1Float -> [[V1Int]] -> Int -> [V1Float] -> [V1Float] -> [V1Float] -> V1Float -> V1Float
feedForward input consols branching ws bs fws fbs= 
    map (tanh) . zipWith (+) fbs  . map (\fw -> vdot initial fw) $ fws
...
predict input consols branching ws bs fws fbs = 
    (\x -> x - 1) . head . (DL.elemIndices (DL.maximum fed)) $ fed
...
intermediateFF input consols branching ws bs fws fbs=  
    ...
    (out, initial)
        where initial = scanl (\inp (w,b,cons) -> layer inp cons branching w b) input $ zip3 ws bs consols
              out = map (tanh) . zipWith (+) fbs . map (\fw -> vdot (last initial) fw) $ fws
...
gradLayer dEwrtInputs prevLayerOutput branching consols ws bs = (dErrordWeights, dEwrtInputs, newdErrordInputs)                     
    where
        ...
        expandedLayerOutput = concatMap (replicate branching) prevLayerOutput :: V1Float
        expandedLayerOutputZeroed = V.replicate ((*branching) $ length prevLayerOutput) 0.0 :: V.Vector Float

        consolandInds = zip [0..(length consols) - 1] consols :: [(Int, V1Int)]
        
        getSublistdEdW :: (Int, V1Int) -> V1Float
        getSublistdEdW (ind,consSublist) = map (\consIndex -> (dEwrtInputs !! ind) * (expandedLayerOutput !! consIndex)) consSublist 
        ...
        inddEdWPairs = V.zip (V.fromList $ concat consols) (V.fromList $ concat dEdWsublists) :: V.Vector (Int, Float)
        dErrordWeights = V.toList $ V.update expandedLayerOutputZeroed inddEdWPairs --ABANDONED HERE
        ...
        dEwrtInputsV = V.fromList dEwrtInputs
        wsV = V.fromList ws
        dEdOutsunplaced= map (\(ind,consSub)-> map (\consInd-> (dEwrtInputsV V.! ind) * (wsV V.! consInd)) consSub) consolandInds
        
        dEdOutIndpairs = V.zip (V.fromList $ concat consols) (V.fromList $ concat dEdOutsunplaced)
        ...
        dEdOuts = map (foldl' (+) 0) $ chunksOf branching $ V.toList unsummeddEdOuts
        newdErrordInputs = zipWith (*) dEdOuts dOutsdIns 
...
grad  inp outp consols branching ws bs fws fbs = [dEdWs, dEdBs, dEdfws, [initdEdIns]]
    where 
        ...
        finaldOutdIn = map (\x -> 1 - x**2) final
        
        initdEdIns = zipWith (*) finaldOutdIn $ zipWith (-) outp final
        finalStructuredLayerOut = last layerOutputs
        dEdfws = map (\dEdIn -> map (*dEdIn) finalStructuredLayerOut) initdEdIns :: V2Float
        -- !dEdfbs = initdEdIns :: V1Float
        
        listifyF2 :: V.Unbox a => ([[a]] -> [[a]]) -> [V.Vector a] -> [V.Vector a]
        listifyF2 f = map V.fromList . f . map (V.toList)
        transposedFws = DL.transpose fws
        
        finalStructuredLayerdOutdIn = map (\x-> 1-x**2) finalStructuredLayerOut :: V1Float
                                    --dOut/dIn                                            dE/dOut = dE/dIn * w
        lastdEdIn = zipWith (*) finalStructuredLayerdOutdIn $ map (vdot initdEdIns) transposedFws
...
descend consols branching inp outp ws bs fws fbsI =
    ...
    in  toTuple $ DL.zipWith (DL.zipWith (DL.zipWith (\g val -> val - g * alpha))) mygrads [ws,bs,fws,fbsI] 
...
instance Entity Genome Float ([[Float]],[[Float]],[[Float]],[[Float]]) () IO where
  ...
  score (trainImages, trainLabels, testImages, testLabels) !genome = do
    ...
    let
        !consols = genConsolidate genome 10 [1.0..784.0]
        branching = length . concat $ funcs genome
    putStrLn "\nconsolidation list generated...\n"
    putStrLn $ "synapse number: " ++ ( show ((length $ last consols) * 10 + (length $ concat $ concat consols)))
    
    
    ws <- evalRandIO $ genWeights $ consols :: IO [[Float]]
    bs <- evalRandIO  $ genBiases consols :: IO [[Float]]
    fws <- evalRandIO . Prelude.sequence $ Prelude.replicate 10 $ genWeight (length $ last consols) :: IO [[Float]] --replicate10 b/c 10 out neurons
    fbs <- evalRandIO $ Prelude.sequence $ Prelude.replicate 1 $ genWeight 10 :: IO [[Float]]
    
    
    putStrLn $ "descending once"
    startTime <- T.getCurrentTime 
    putStrLn $ show $ map (map (length)) $ toList4 $ descend consols branching (head trainImages) (head trainLabels) ws bs fws fbs
    ...
    -- train:: [[[Float]]] -> [[[Float]]]
    let train [w,b,fw,fb] =  foldl' (\(!tw,!tb,!tfw,!tfb) (inp,outp) -> descend consols branching inp outp tw tb tfw tfb) (w,b,fw,fb) $ zip trainImages trainLabels -- zipped
    ...
    putStrLn $ "-Debug " ++ (show $ map ((map (foldl' (+) 0) ) ) weightlist)
    putStrLn "Testing Genome..."
    
    let getTestAccuracy w b fw fb = foldl' (\acc (tI,tL)-> if (predict tI consols branching w b fw fb) == (head $ elemIndices 1.0 tL) then acc + 1 else acc ) 0 $ zip testImages testLabels :: Int
