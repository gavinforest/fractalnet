...
layer input consol branching w b =  V.map (tanh) $ V.zipWith (+) b $ (\x -> floatConsolVec x consol) $ V.zipWith (*) w branched
...
feedForward input consols branching ws bs fws fbs= 
    V.map (tanh) . V.zipWith (+) fbs . V.fromList . map (\fw -> vdot initial fw) $ fws
...
intermediateFF input consols branching ws bs fws fbs=  
    ...
    (out, initial)
        where initial = scanl (\inp (w,b,cons) -> layer inp cons branching w b) input $ zip3 ws bs consols
              out = V.map (tanh) . V.zipWith (+) fbs . V.fromList . map (\fw -> vdot (last initial) fw) $ fws
...
gradLayer dEwrtInputs prevLayerOutput branching consols ws bs = (dErrordWeights, dEwrtInputs, newdErrordInputs)                     
    where
        ...
        dErrordWeights = V.update_ expandedLayerOutputZeroed (V.concat consols) (V.concat dEdWsublists)
        ...
        dOutsdIns = V.map (\x -> 1 /((cosh $ atanh x)**2)) prevLayerOutput --remember dOut/dIn = 1 - (tanh in)**2 = 1- out**2
        ...
        dEdOutIndpairs = V.zip (V.concat consols) (V.concat dEdOutsunplaced)
...
grad  inp outp consols branching ws bs fws fbs = [dEdWs, dEdBs, dEdfws, [initdEdIns]]
    where 
        ...
        finaldOutdIn = V.map (\x -> 1/((cosh $ atanh x)**2)) final
        ...
        finalStructuredLayerdOutdIn = V.map (\x-> 1/((cosh $ atanh x)**2)) finalStructuredLayerOut :: V1Float
...
instance Entity Genome Float ([[Float]],[[Float]],[[Float]],[[Float]]) () IO where
  ...
  score (trainImages, trainLabels, testImages, testLabels) !genome = do
    ...
    -- train:: [[[Float]]] -> [[[Float]]]
    let train [w,b,fw,fb] (trains, labels) =  foldl' (\(!tw,!tb,!tfw,!tfb) (inp,outp) -> descend consols branching (V.fromList inp) (V.fromList outp) tw tb tfw tfb) (w,b,fw,fb) $ zip trainImages trainLabels -- zipped
    
    myZipped <- return $ chunksOf 50000 $ (\a -> R.shuffle' a (length a) (mkStdGen 1123512)) $ concat $ replicate epochs $ zip trainImages trainLabels
    ...
    writeFile ("unboxedIndividual0.002Beta" ++ fileName) ((show errors) ++ " \n\n\n" ++ (show genome))
