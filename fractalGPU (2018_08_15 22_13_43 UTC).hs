...
type V1Float = A.Acc (A.Vector Float)
type V2Float = V.Vector (A.Acc (A.Vector Float))
type V3Int = V.Vector (V.Vector (A.Acc (A.Vector Int)))
type V2Int = V.Vector (A.Acc (A.Vector Int))
...
floatConsolVec inps conslist= V.map (\inds -> A.fold (+) 0.0 $ A.map (\ind -> inps A.! ind) inds) conslist
...
descend consols branching inp outp ws bs fws fbsI =
    ...
    in  toTuple $ DL.zipWith (V.zipWith (V.zipWith (\g val -> val - g * alpha))) mygrads [ws,bs,fws,fbsI] 
...
instance Entity Genome Float [[[Float]]] () IO where
  ...
  score [trainImages, trainLabels, testImages, testLabels] genome = do
    ...
    let
        !cons = V.fromList $ map (V.fromList) $ map (map V.fromList) $ genConsolidate genome 10 [1.0..784.0]
    ...
    ws <- return $ list2toVec2 wsTemp :: IO (V2Float)
    bsTemp <- evalRandIO  $ genBiases consList :: IO [[Float]]
    bs <- return $ list2toVec2 bsTemp ::  IO (V2Float)
    fwsTemp <- evalRandIO . Prelude.sequence $ Prelude.replicate 10 $ genWeight (V.length $ V.last cons) :: IO [[Float]] --replicate10 b/c 10 out neurons
    fws <- return $ list2toVec2 fwsTemp :: IO (V2Float)
    fbsTemp <- evalRandIO $ Prelude.sequence $ Prelude.replicate 1 $ genWeight 10 :: IO [[Float]]
    fbs <- return $ list2toVec2 fbsTemp :: IO (V2Float)
    
    putStrLn "descending once"
    startTime <- T.getCurrentTime
    putStrLn $ show $ map (V.map (V.sum)) $ toList4 $ descend cons branching (V.fromList $ head trainImages) (V.fromList $ head trainLabels) ws bs fws fbs
    endTime <- T.getCurrentTime
    putStrLn $ "finished descent" ++ (show $ T.diffUTCTime endTime startTime)
    
    putStrLn $ "\nstarting to score genome: " ++ (show genome)
    -- zipped <- return $ concat $ replicate (round $ epochs/10) $ zip trainImages (trainLabels :: [V1Float])
    -- train:: [[[Float]]] -> [[[Float]]]
    let train [w,b,fw,fb] =  foldl' (\(tw,tb,tfw,tfb) (inp,outp) -> descend cons branching (V.fromList inp) (V.fromList outp) tw tb tfw tfb) (w,b,fw,fb) $ zip trainImages trainLabels -- zipped
    ...
    putStrLn $ "-Debug " ++ (show $ map ((V.map (V.sum) ) ) weightlist)
    putStrLn "Testing Genome..."
    
    let getTestAccuracy w b fw fb = foldl' (\acc (tI,tL)-> if (predict (V.fromList tI) cons branching w b fw fb) == (head $ elemIndices 1.0 tL) then acc + 1 else acc ) 0 $ zip testImages testLabels :: Int
